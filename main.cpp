...
#define OUTPUT "_out.txt"

std::vector<std::string> extractDialogLines(const std::string& content)
{
...
void convertTxtToDtl(const std::filesystem::path& txtFilePath)
{
    std::ifstream inputFile(txtFilePath);
    std::ofstream outputFile((txtFilePath.stem() += OUTPUT));
